...
const updateUserAvatar = asyncHandler(async(req,res) => {
    ...
    //Delete previous avatar from cloudinary first
    const prevUser = await User.findById(req.user?._id).select("avatar");
    if(!prevUser){ throw new ApiError(404,"User not found");}

    if (prevUser.avatar) {
    const publicId = extractPublicId(prevUser.avatar);
...
const updateUserCoverImage = asyncHandler(async(req,res) => {
    ...
    //Delete previous cover image from cloudinary first
    const prevUser = await User.findById(req.user?._id).select("coverImage");
    if(!prevUser){ throw new ApiError(404,"User not found");}

    if (prevUser.coverImage) {
    const publicId = extractPublicId(prevUser.coverImage);
    await deleteFromCloudinary(publicId);
  }
